...
## 진입점
- `node .harness/module-gate.mjs [--staged|--base <ref>|--fix]` — .harness/module-gate.mjs:431,435 (판정 결과를 종료 코드로 낸다)
- `node --test .harness/module-gate.test.mjs` — .harness/module-gate.test.mjs:16 (게이트의 회귀 테스트 33개)
...
## 불변식
- I1. 판정 수준은 `status` 로만 갈린다 — `active` 는 FAIL, 그 외는 WARN 으로 강등된다. 예외는 R3 의 모듈 후보 경고 하나로 언제나 WARN 이다 (근거: .harness/module-gate.mjs:202-203,246-248, .harness/module-gate.test.mjs:126,459) [테스트]
- I2. 차단 수단은 종료 코드뿐이다 — FAIL 이 하나라도 있으면 1, 없으면 0 이고 WARN 은 0 이다 (근거: .harness/module-gate.mjs:431,435, tools/git-hooks/pre-commit:95-106) [grep]
...
- I5. 게이트는 자기 판단을 파일에 쓰지 않는다 — 유일한 쓰기는 `--fix` 의 근거 경로 정정이고 그것도 `근거:` 가 있는 줄만 건드린다 (근거: .harness/module-gate.mjs:413-417,424) [리뷰]
...
- R13 의 건수 주장 예산 24개에 리포가 이미 닿는다 — 지금 23개이고, 넘으면 넘은 지점부터 조용히 세지 않는다. 2026-09-12 editor 계약서를 세우는 동안 실제로 한 번 넘었고 루트의 묘비 셋이 주장을 지워 다시 내려갔다. 예산을 올릴지, 주장을 모듈별로 나눠 셀지 미결
