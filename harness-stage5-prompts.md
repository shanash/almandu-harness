...
## 5e-0 — 05 관찰 문서 개설 (module-harness, 한 번만)

```
module-harness 의 5단계 리뷰 작업이다. 5a~5d 가 끝나 패킷·페르소나·verdict·커밋 trailer 가 모두 동작하고
v0.4.1 이 찍혀 있다. 5e 는 실제 작업 커밋 10건 이상에 리뷰를 태우는 기간 관찰인데, 그 기록을 받을
`observations/05-review-field.md` 가 아직 없다. 지금 할 일은 그 문서를 개설하고, **이미 관찰된 것을
그 안으로 이관**하는 것이다. 새 관찰을 만들지 않는다 — 있는 것을 옮기고 셈을 맞춘다.

먼저 읽을 것 (이 순서로):
1. DESIGN-review.md — 6절(승격 경로), 8절(관찰 기록 형식), 10절(종료 조건 셋)
2. observations/04-review-observation.md — 프로브·정상 커밋 표, 재실행 절
3. 5a~5d 의 커밋 로그와, 그 커밋들의 `Review:` trailer (`git log --format='%H %s%n%b'`)
   — 5b·5c·5d 세션이 자기 변경을 루프로 커밋했으므로 trailer 가 붙은 커밋이 이미 있다

할 일:

[1] `observations/05-review-field.md` 를 만든다. 골격은 이렇게:

    # 05 리뷰 실전 관찰 — 5e
    (한 줄: 이 문서는 실제 작업 커밋의 verdict 를 누적한다. 프로브는 04 에 있고 여기 섞지 않는다)

    ## 종료 조건 현황  ← 매 세션 끝에 갱신하는 표. 3행 고정
    | 조건 | 기준 | 현재 | 충족 |
    | 실전 검출 | 1건 이상 (프로브 아님) | ? | ? |
    | 승격 경로 | 규칙 후보 1건 또는 질문 내림 1건 | ? | ? |
    | 오탐률 | 정상 커밋 10건 중 1건 이하 | ?/? | ? |

    ## 페르소나 연속 통과 카운터  ← 10회면 질문 내림 후보 (DESIGN-review 6절)
    | 페르소나 | 연속 통과 | 마지막 비통과 커밋 |

    ## 관찰 표  ← 세션마다 한 줄
    | 날짜 | 커밋 | 계약대조 | 불변식 | 범위 | 검출/오탐/통과 | 이유 요약 |

    ## 비통과 verdict 전문  ← "아니오/거짓" 의 reason·evidence 를 요약하지 말고 그대로
    ## 규칙 후보  ← 같은 이유 3건 모이면 여기 + DESIGN-review 미결
    ## 하네스 결함 메모  ← 관찰 중 발견한 패킷·페르소나·게이트 결함. 5e 중에는 고치지 않는다
    ## 결론  ← (5e 종료 시 작성)

[2] 이미 관찰된 것을 이관한다. **04 를 수정하지 않는다.** 읽고 옮겨 적는다.
    - 5b 가 낸 실전 검출 1건을 "관찰 표" 와 "비통과 verdict 전문" 에 적는다. 어느 커밋인지
      trailer 로 특정하고, 프로브가 아니라 실제 변경이었음을 한 줄로 근거를 적는다
    - 범위 감시자의 연속 통과 7회가 어느 커밋들에서 나온 것인지 trailer 를 훑어 특정하고,
      카운터 표에 7 과 마지막 비통과 커밋을 적는다. **7 이 재현되지 않으면 세션을 멈추고 보고한다**
      — 셈이 근거 없이 시작되면 10회 판단이 무의미하다
    - 계약 대조자·불변식 판정자의 연속 통과도 같은 방식으로 센다
    - 이관한 커밋들은 관찰 표에 날짜순으로 넣는다. 5a~5d 는 하네스 자기 작업이므로
      "자기 커밋" 으로 표시하고, 정상 커밋 표본 수(오탐률 분모)에는 넣을지 여부를 판단해 적는다
      — 넣지 않기를 권한다(하네스 자신을 고치는 커밋은 계약 밀도가 실제 프로젝트와 다르다)

[3] 종료 조건 현황 표를 현재 값으로 채운다. "?" 를 남기지 않는다. 근거는 [2] 의 이관 결과다.

[4] DESIGN-review.md 의 상태 기록을 갱신한다 — 5a~5d 완료, 5e 진행 중, 05 문서 개설.
    필드를 늘리거나 종료 조건을 고치지 않는다.

[5] 루프로 커밋한다 (scope → change → reconcile → /module-review → commit).
    이 커밋 자체도 05 관찰 표에 "자기 커밋" 으로 한 줄 들어간다.

지킬 것:
- 04 의 프로브 결과를 05 로 옮기지 않는다. 프로브와 실전은 분모가 다르다
- 04 를 수정하지 않는다. 원본 관찰은 불변이다
- 새 verdict 를 만들지 않는다. 이 세션의 커밋 하나를 제외하면 전부 이관이다
- 페르소나·패킷·게이트를 고치지 않는다. 결함이 보이면 "하네스 결함 메모" 에 적는다
- 카운터를 추정으로 채우지 않는다. trailer 로 특정되지 않으면 그 사실을 적는다

끝나는 조건: 05 가 있고, 종료 조건 3행에 "?" 가 없고, 연속 통과 카운터가 trailer 로 특정된 근거를 갖고,
실전 검출 1건의 verdict 전문이 05 에 있고, 이 세션의 커밋이 루프를 통과했다.
```
