...
이 절은 위 본문을 고치지 않고 덧붙인 것이다. 원본 표와 원본 서술은 그대로 둔다 — 무엇을 잘못 셌는지가
기록이고, 고치면 그 기록이 사라진다. 아래는 판단이 아니라 사실 확인이다.

### 전제 A — "실전 검출 1건(5b)" 은 재현되지 않는다 (본문 177·181행)

두 리포에서 `Review:` trailer 를 가진 커밋은 10건이고 실제 답이 박힌 것은 9건인데 **전부 통과**다
(`Review: none` 1건 제외). 비통과 verdict 전문은 두 리포 어디에도 없다. 지목된 5b 커밋(589c5bd)의
trailer 도 `계약대조=예 불변식=참(1) 범위=예` 다. 원천인 `.git/module-loop/` 는 루프가 커밋과 함께
비우므로 지금은 확인할 방법이 없다.

검출이 실제로 있었을 수는 있다. 그러나 **확인이 불가능하고, 확인할 수 없는 것은 세지 않는다.**

→ 본문 181행의 "종료 조건(10절) 셋 중 지금 채워진 것은 '실전 검출 1건'(5b) 하나다" 는 **무효**다.
   종료 조건 1 은 **미충족**으로 정정한다. 다른 두 조건도 그대로 미충족이다.

### 전제 B — "범위 감시자 연속 통과 7회" 는 연속이 아니다 (본문 203행)

7 은 **누계**다. 연속이 아니고, 모집단도 섞여 있다.

- 되돌린 프로브는 커밋되지 않아 trailer 가 없다. 정의상 나중에 특정할 수단이 없는 실행이다
- 시간순으로 세면 1차는 P1(예) P2(예) P3(**아니오**) 정상 3건(예), 재실행은 P1(예) P2(예) P3(**아니오**) 다.
  마지막 비통과가 P3 재실행이므로 **그 이후의 연속 통과는 0** 이다
- 산술: 1차 "4회" 는 실제로 `예` 5개이고(P1·P2·정상 3건 — 본문 131행이 `아니오` 를 그 목록에 넣지 않은 채
  개수만 5에서 4로 적었다), 재실행 "3" 은 실제로 `예` 2개다(P1·P2). 누계 7 은 5+2 라 우연히 맞지만
  내역은 둘 다 틀렸다. 본문 148행이 적은 "(5/5)" 가 1차의 실제 수다

→ 본문 203행의 "통과 연속 횟수는 이제 7회다(1차 4 + 재실행 3)" 는 **무효**다. 05 는 0 에서 시작한다.

### 왜 이런 오류가 났는가

프로브 결과(되돌려져 커밋이 없다)와 실전 결과(trailer 가 있다)를 **같은 셈에 넣었다.**
두 모집단은 재현 수단이 다르다 — 하나는 사후에 특정할 수 없고 하나는 grep 으로 셀 수 있다.
섞으면 어느 쪽도 검증되지 않는다. DESIGN-review 8절이 "검출 / 오탐 / 페르소나 결함 셋으로 가른다,
섞으면 셋 다 안 고쳐진다" 고 정한 이유가 여기서 실증됐다.

05 는 모집단을 커밋 trailer 하나로 닫고 시작한다 (`observations/05-review-field.md` 의 "셈의 규칙").
